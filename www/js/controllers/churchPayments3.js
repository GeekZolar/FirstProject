...
gtw.controller('churchPayments3Ctrl', function ($scope,$state,ACTION_URLS,Gtbank,CallService,UtilityService,$ionicHistory) {
  ...
  $scope.form.data.PostFormFields = formReview
                                    .map(function(event){return {FieldId: event.field_id, ActualValue: event.actual_value}} );
  // var amount = $scope.formReview
  //                .filter((event) => event.amt_ref === "1")
  //                .map((event) => event.actual_value);
  //
  //  $scope.form.Others.Amount = parseFloat(amount[0])+"";

  var amount = formReview.filter(function (event) {return event.amt_ref === "1"});

    var amt = amount.map(function(event){return event.actual_value});
  ...
  $scope.enableAuthorization = function(){
    ...
    $scope.form.reviewName = 'Church Payment';
    $scope.form.Token = "";
