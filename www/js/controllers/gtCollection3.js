...
gtw.controller('GTCollCtrl_3', function ($scope,$state,ACTION_URLS,Gtbank,CallService,UtilityService,$ionicHistory) {
  ...
  // var amount = $scope.formReview.filter((event) => event.amt_ref === "1").map((event) => event.actual_value);
  //
  ...
  $scope.enableAuthorization = function(){
    ...
    $scope.form.reviewName = 'GTCollections';
    $scope.form.Token = "";
