...
gtw.controller('utilitytzCtrl', function (Idle, $scope,$state,ACTION_URLS,Gtbank,CallService,UtilityService,$ionicHistory) {
  ...
  $scope.showAmount = false;
      ...
      $scope.getCategory = function() {
        ...
        $scope.getDataBundleTypes = function() {
          ...
          if ($scope.productId.Airtime == "DSTV") {
            $scope.shwsub = true;
            $scope.shwref = false;
          }
          else {
    ...
    $scope.confirmTransfer = function() {
      ...
      // $scope.form.data.ProductId = $scope.form.Others.CustomerMobile;
      ...
      CallService.doPostWithoutEnc($scope.form.data, ACTION_URLS.AIRTIME)
        .then(function (response) {
         ...
         var res = angular.fromJson(response.data);
          if (res.StatusCode == 0) {
            var trnsResp = angular.fromJson(res.Message);
            // var trnsResp = angular.fromJson('Your request has been submitted successfully');
            if (trnsResp.CODE == "1000") {
